...
function possibleRights(count:number,amount:number):boolean {
 // The verified ledger supplies the amounts. An old account may use another
 // rate, so this reader checks consistency without recreating its contract.
 return integer(count)&&integer(amount)&&(count!==0||amount===0);
}
...
export function validCommissionEvidence(v:unknown):v is CommissionEvidence {
 if(!object(v)||!ledgerUuid(v.id)||!['monthly_paid','annual_once'].includes(v.kind)
  ||!integer(v.amount_cents)||v.currency!=='EUR'||!LEDGER_STATES.includes(v.status)
