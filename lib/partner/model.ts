...
/** This exact, published document states the 5 € / 50 € programme. An account's
 * older terms, a draft, or a locally rendered preview cannot establish its rate. */
const fixedRateTerms={
  version:'2026-09-12.1',
  sha256:'5afd08fc14a23ebebe596951ba058d0140e3c40c9d61101f58a33fcf986acdc4',
  url:'https://partners.foreas.xyz/programme-partenaire/conditions/2026-09-12.1',
} as const;
export function fixedRatePolicyVerified(me:PartnerMe|null,terms:PartnerTerms|null):boolean {
  const document=terms?.document;
  return !!me && !!document && !me.terms.acceptance_required
    && me.terms.required_version===fixedRateTerms.version
    && me.terms.accepted_version===fixedRateTerms.version
    && terms.accepted_version===fixedRateTerms.version
    && document.version===fixedRateTerms.version
    && document.sha256===fixedRateTerms.sha256
    && document.document_url===fixedRateTerms.url;
}
...
export function referralForSharing(_me:PartnerMe|null):string|null {
  // The live /r/<code> offer lookup is not verified in production.
  return null;
}
