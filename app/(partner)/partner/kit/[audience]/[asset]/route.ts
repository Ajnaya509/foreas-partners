...
import {previewEnabled,referralForSharing} from '@/lib/partner/model';
import {getAudience,kitAssets,kitId,KIT_VERSION,NEW_PARTNER_TERMS_VERSION,kitDocument,messagePack,type KitCommissionPolicy} from '@/lib/partner/kit';
...
export async function GET(request:NextRequest,{params}:{params:Promise<{audience:string;asset:string}>}){
  ...
  let commissionPolicy:KitCommissionPolicy=preview?'second_paid_month':'legacy';
  if(!preview){
    ...
    if(me.data.terms.accepted_version===NEW_PARTNER_TERMS_VERSION)commissionPolicy='second_paid_month';
